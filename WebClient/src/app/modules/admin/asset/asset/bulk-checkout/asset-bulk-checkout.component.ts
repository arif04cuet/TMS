...
@Component({
  selector: 'app-asset-bulk-checkout',
  templateUrl: './asset-bulk-checkout.component.html'
})
export class AssetBulkCheckoutComponent extends FormComponent {
  ...
  @ViewChild('bulkAssetSelect') bulkAssetSelect: SelectControlComponent;
  ...
  ngOnInit(): void {
    ...
    this.createForm({
      assetIds: [null, [], this.v.required.bind(this)],
  ...
  ngAfterViewInit() {
    ...
    this.bulkAssetSelect.register((pagination, search) => {
      const checkoutToAssetId = this.form.controls.chekoutToAssetId.value;
      search = search || '';
      search += `&Search=CheckoutId eq NULL`;
      if (checkoutToAssetId) {
        search += `&Search=Id ne ${Number(checkoutToAssetId)}`
      }
      return this.assetHttpService.list(pagination, search);
    }).fetch();
  }
